...
module move_book_code::my_odd_coin {
    use std::signer;
    use move_book_code::base_coin;
    ...
    public fun setup_and_mint(account: &signer, amount: u64) {
        base_coin::publish_balance<MyOddCoin>(account);
        base_coin::mint<MyOddCoin>(signer::address_of(account), amount, MyOddCoin {});
    }

    public fun transfer(from: &signer, to: address, amount: u64) {
        // amount must be odd.
        assert!(amount % 2 == 1, ENOT_ODD);
        base_coin::transfer<MyOddCoin>(from, to, amount, MyOddCoin {});
    }
    ...
    */
    ...
    fun test_odd_success(from: signer, to: signer) {
        ...
        assert!(base_coin::balance_of<MyOddCoin>(@0x42) == 35, 0);
        assert!(base_coin::balance_of<MyOddCoin>(@0x10) == 17, 0);
    }
